...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const handleWishlistClick = (e: React.MouseEvent) => {
    e.stopPropagation(); // Evita que al hacer clic en el corazón se abra la ficha del producto
    dispatch(toggleWishlist(product));

    // Disparar alertas  basadas en el estado previo
    if (!isFavorite) {
      toast.success(`${product.name.toUpperCase()} ADDED TO WISHLIST`, {
        
      });
    } else {
      toast.success(`${product.name.toUpperCase()} REMOVED FROM WISHLIST`, {
